fetch("http://localhost:3000/api/cameras").then(response =>{
    ...
    //recup_valeur_json=JSON.parse(localStorage.getItem(ligne_commande));
    //commande.innerHTML=((recup_valeur_json).toString()).replace(/,/g,"");
    commande_en_cours++;
    ligne_commande= "ligne"+commande_en_cours;
    ...
    while (x<=commande_en_cours) {
       ...
       /* commande.innerHTML+='<p>ligne'+x+'</p>';*/
...
//-------------------------------------------------------------------------------------//
//################action après avoir fait son choix de produit#####################
//-----------------------------------------------------------------------------------//
